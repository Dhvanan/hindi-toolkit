...
def viterbi(observations,tags,initial_p,transition_p,emission_p):
	...
	if(observations[0] not in emission_p):
		...
		initial_tag,initial_value = sorted(initial_p.items(), key = operator.itemgetter(1),reverse = True)[0]
	...
	for i in range(1,len(observations)):
		...
		if(observations[i] not in emission_p):
			...
			for t in t1[i-1]:
				...
				#Get the most likely transition for the prev tag and add the cumulative values.
				tag,value = sorted(transition_p[t].items(), key = operator.itemgetter(1), reverse = True)[0]
		...
		#KNOWN Word
			...
			for cur_tag in emission_p[observations[i]]:
				...
				#Calculate most likely previsous tag for cur_tag from prev_tags (Including transitions and emissions)
				for prev_tag in t1[i-1]:
					calculated_value = transition_p[prev_tag][cur_tag] + emission_p[observations[i]][cur_tag] + t1[i-1][prev_tag]
	...
	final_tags = []
	final_tag,final_max_value = sorted(t1[-1].items(), key = operator.itemgetter(1), reverse = True)[0]
...
def pos_tag(text):
	with open('pos/hmmmodel.txt','r') as f1:
	...
	for sentence in tokenized_data:
		tags = viterbi(sentence,tags,initial_p,transition_p,emission_p)
		tagged_sentence = tag_sentence(tags,sentence)
